...
async function main() {
  ...
  for (const ds of datasets) {
    ...
    amendRows += aN;
    partyRows += pN;
    process.stderr.write(`   ${cN.toLocaleString('en-US')} contracts, ${aN.toLocaleString('en-US')} amendments, ${pN.toLocaleString('en-US')} parties\n`);
  }

  cOut.end();
  aOut.end();
  pOut.end();
  await Promise.all([once(cOut, 'finish'), once(aOut, 'finish'), once(pOut, 'finish')]);
  process.stderr.write(
    `==> wrote ${contractRows.toLocaleString('en-US')} contracts + ${amendRows.toLocaleString('en-US')} amendments + ` +
      `${partyRows.toLocaleString('en-US')} parties (largest stmt ${Math.max(cb.max(), ab.max(), pb.max()).toLocaleString('en-US')} bytes)\n`,
  );
  ...
  process.stderr.write(`\n==> done: ${contractRows} contracts, ${amendRows} amendments, ${partyRows} parties\n`);
}
