...
// sensitive natural-person ID) → null, the trading displayName stays PUBLIC (ADR-0007 §6), and the
...
describe('contract.data loader — natural-person bidder branch', () => {
  it('clears the sole-trader ЕИК and marks the response noindex (covers HTML + .data twin)', async () => {
    ...
    // The loader must NOT emit X-Robots-Tag directly — that is the worker's job (ADR-0008).
